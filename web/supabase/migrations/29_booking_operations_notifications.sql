-- Afrofade Database Migration 29: booking operations + notification outbox
...
BEGIN
  ...
  IF b.customer_user_id=auth.uid() THEN
    allowed := p_to_status='cancelled_by_customer' AND b.status IN ('requested','confirmed');
  ELSE
    IF b.target_type='professional' THEN
      SELECT id INTO pro FROM public.professional_profiles WHERE id=b.professional_profile_id AND user_id=auth.uid();
      allowed := pro IS NOT NULL;
    ELSE
      SELECT * INTO member FROM public.salon_memberships WHERE salon_id=b.salon_id AND user_id=auth.uid() AND status='active' AND role IN ('owner','manager','professional') LIMIT 1;
      allowed := FOUND AND (member.role IN ('owner','manager') OR member.professional_profile_id=b.assigned_professional_profile_id);
    END IF;
    IF allowed THEN
      allowed := CASE b.status
        WHEN 'requested' THEN p_to_status IN ('confirmed','rejected','cancelled_by_provider')
        WHEN 'confirmed' THEN p_to_status IN ('completed','cancelled_by_provider','no_show_customer','no_show_provider')
        ELSE FALSE END;
    END IF;
  END IF;
  ...
  IF p_note IS NOT NULL AND btrim(p_note)<>'' THEN
    UPDATE public.booking_status_events SET note=left(p_note,1000) WHERE id=(SELECT max(id) FROM public.booking_status_events WHERE booking_id=b.id);
  END IF;
  RETURN b;
END $$;
