-- Afrofade Database Migration 30: saved looks + booking visual brief
-- BMAD Story 14.7
...
BEGIN
  ...
  SELECT COALESCE(t.label_fr,h.name,l.title) INTO style_label
  FROM public.saved_looks x
  LEFT JOIN public.hair_taxonomy t ON t.id=x.taxonomy_id
  LEFT JOIN public.hairstyles_catalog h ON h.id=x.legacy_style_id
  WHERE x.id=l.id;
  UPDATE public.marketplace_bookings SET saved_look_id=l.id,visual_brief_snapshot=jsonb_build_object(
    'title',l.title,'style',COALESCE(style_label,l.title),'legacyStyleId',l.legacy_style_id,'taxonomyId',l.taxonomy_id,'customization',l.customization
  ),updated_at=NOW() WHERE id=b.id RETURNING * INTO b;
  RETURN b;
END $$;

CREATE OR REPLACE FUNCTION public.enforce_saved_look_head_ownership()
RETURNS TRIGGER LANGUAGE plpgsql SET search_path=public AS $$
BEGIN
  IF NEW.customer_head_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM public.customer_heads h WHERE h.id=NEW.customer_head_id AND h.user_id=NEW.user_id) THEN RAISE EXCEPTION 'saved_look_head_not_owned'; END IF;
  RETURN NEW;
END $$;
...
DROP POLICY IF EXISTS saved_looks_own_select ON public.saved_looks;
CREATE POLICY saved_looks_own_select ON public.saved_looks FOR SELECT TO authenticated USING(user_id=auth.uid());
DROP POLICY IF EXISTS saved_looks_own_insert ON public.saved_looks;
CREATE POLICY saved_looks_own_insert ON public.saved_looks FOR INSERT TO authenticated WITH CHECK(user_id=auth.uid());
DROP POLICY IF EXISTS saved_looks_own_update ON public.saved_looks;
CREATE POLICY saved_looks_own_update ON public.saved_looks FOR UPDATE TO authenticated USING(user_id=auth.uid()) WITH CHECK(user_id=auth.uid());
DROP POLICY IF EXISTS saved_looks_own_delete ON public.saved_looks;
CREATE POLICY saved_looks_own_delete ON public.saved_looks FOR DELETE TO authenticated USING(user_id=auth.uid());
...
REVOKE ALL ON FUNCTION public.attach_saved_look_to_booking(UUID,UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.attach_saved_look_to_booking(UUID,UUID) TO authenticated,service_role;
