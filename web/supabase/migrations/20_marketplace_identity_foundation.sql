-- Afrofade Database Migration 20: marketplace identity foundation
-- BMAD Story 12.1 — additive identity model beside legacy customer/salon/admin roles.
-- Marketplace migrations use the 20+ range to avoid colliding with parallel 3D migrations.
--
-- IMPORTANT:
...
CREATE TABLE IF NOT EXISTS public.professional_profiles (
    ...
    CONSTRAINT professional_profiles_operating_mode_check CHECK (
        operating_mode IS NULL OR operating_mode IN ('independent', 'mobile', 'studio', 'hybrid', 'salon_only')
    ),
    CONSTRAINT professional_profiles_job_seeking_status_check CHECK (
        job_seeking_status IN ('not_looking', 'open', 'actively_looking')
    ),
    CONSTRAINT professional_profiles_verification_status_check CHECK (
        verification_status IN ('unverified', 'pending', 'verified', 'rejected', 'suspended')
    ),
    CONSTRAINT professional_profiles_listing_status_check CHECK (
        listing_status IN ('draft', 'published', 'paused', 'suspended')
    ),
    CONSTRAINT professional_profiles_service_radius_check CHECK (
        service_radius_m IS NULL OR service_radius_m BETWEEN 0 AND 500000
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_professional_profiles_slug_ci
    ON public.professional_profiles (LOWER(slug)) WHERE slug IS NOT NULL;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.salons ADD CONSTRAINT salons_verification_status_check
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.salons ADD CONSTRAINT salons_listing_status_check
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.salons ADD CONSTRAINT salons_booking_confirmation_mode_check
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_salons_slug_ci
    ON public.salons (LOWER(slug)) WHERE slug IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_salons_listing_status ON public.salons (listing_status);
CREATE INDEX IF NOT EXISTS idx_salons_verification_status ON public.salons (verification_status);
...
CREATE TABLE IF NOT EXISTS public.salon_memberships (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT salon_memberships_role_check CHECK (role IN ('owner', 'manager', 'professional')),
    CONSTRAINT salon_memberships_status_check CHECK (status IN ('invited', 'active', 'suspended', 'ended')),
    CONSTRAINT salon_memberships_permissions_object_check CHECK (jsonb_typeof(permissions) = 'object'),
    CONSTRAINT salon_memberships_ended_at_check CHECK (
        (status = 'ended' AND ended_at IS NOT NULL)
        OR (status <> 'ended' AND ended_at IS NULL)
    )
);

CREATE INDEX IF NOT EXISTS idx_salon_memberships_user_id ON public.salon_memberships (user_id);
CREATE INDEX IF NOT EXISTS idx_salon_memberships_salon_id ON public.salon_memberships (salon_id);
CREATE INDEX IF NOT EXISTS idx_salon_memberships_professional_profile_id
    ON public.salon_memberships (professional_profile_id) WHERE professional_profile_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_salon_memberships_live_user
    ON public.salon_memberships (salon_id, user_id) WHERE status <> 'ended';
...
AS $$
BEGIN
    IF NEW.professional_profile_id IS NULL THEN RETURN NEW; END IF;
    IF NOT EXISTS (
        SELECT 1 FROM public.professional_profiles p
        WHERE p.id = NEW.professional_profile_id AND p.user_id = NEW.user_id
    ) THEN
...
DROP TRIGGER IF EXISTS trg_membership_professional_ownership ON public.salon_memberships;
CREATE TRIGGER trg_membership_professional_ownership
    BEFORE INSERT OR UPDATE OF user_id, professional_profile_id ON public.salon_memberships
    FOR EACH ROW EXECUTE FUNCTION public.enforce_membership_professional_ownership();
...
CREATE TRIGGER trg_professional_profiles_updated_at
    ...
    FOR EACH ROW EXECUTE FUNCTION public.touch_marketplace_identity_updated_at();
DROP TRIGGER IF EXISTS trg_salon_memberships_updated_at ON public.salon_memberships;
...
DROP POLICY IF EXISTS salon_memberships_select_own ON public.salon_memberships;
CREATE POLICY salon_memberships_select_own ON public.salon_memberships
    FOR SELECT TO authenticated USING (auth.uid() = user_id);
