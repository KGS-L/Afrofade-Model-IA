...
BEGIN
 ...
 INSERT INTO public.marketplace_reports(reporter_user_id,entity_type,entity_id,reason,details)
 VALUES(auth.uid(),p_entity_type,p_entity_id,left(p_reason,60),left(p_details,3000)) RETURNING id INTO rid;
 RETURN rid;
END$$;
...
REVOKE ALL ON FUNCTION public.submit_verified_review(UUID,TEXT,INT,TEXT),public.submit_marketplace_report(TEXT,UUID,TEXT,TEXT) FROM PUBLIC;
