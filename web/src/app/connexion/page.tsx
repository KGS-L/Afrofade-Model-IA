    ...
    <div className="min-h-screen bg-cream text-ink flex flex-col">
      <header className="border-b border-ink/10">
        <div className="max-w-container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link href="/" className="min-h-[44px] inline-flex items-center gap-2 text-sm font-medium text-ink-soft hover:text-terracotta transition-colors">
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-ink-soft">Connectez-vous pour accéder à votre espace Afrofade et à vos achats.</p>
          </div>
          ...
          {method === 'choix' ? (
            <div className="rounded-card border border-ink/10 bg-card shadow-soft p-6 sm:p-8 space-y-4">
              <button type="button" onClick={handleGoogle} disabled={busy} className="w-full min-h-[52px] rounded-pill border-[1.5px] border-ink/15 bg-cream hover:bg-terracotta-wash font-bold text-sm inline-flex items-center justify-center gap-3 transition-colors disabled:opacity-60">
              ...
              <form onSubmit={handleSendOtp} className="space-y-3">
                ...
                <div className="flex items-center gap-2 min-h-[48px] border border-ink/15 rounded-input bg-cream px-4 focus-within:border-terracotta">
                  ...
                  <input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="vous@exemple.com" className="w-full bg-transparent text-sm focus:outline-none" />
                </div>
                <button type="submit" disabled={busy || !email.includes('@')} className="w-full min-h-[52px] rounded-pill bg-terracotta hover:bg-terracotta-dark text-white font-bold text-sm inline-flex items-center justify-center gap-2 shadow-soft transition-colors disabled:opacity-60">
            ...
            <form onSubmit={handleVerifyOtp} className="rounded-card border border-ink/10 bg-card shadow-soft p-6 sm:p-8 space-y-5">
              <div className="text-center space-y-1">
                <div className="w-12 h-12 mx-auto rounded-full bg-terracotta-wash border border-terracotta/40 flex items-center justify-center text-terracotta"><Mail className="w-5 h-5" /></div>
                <p className="text-sm font-bold">Code envoyé à {email}</p>
              ...
              <div className="flex justify-center gap-2" role="group" aria-label="Code OTP">
                {otp.map((digit, i) => (
                  <input key={i} ref={(el) => { inputRefs.current[i] = el; }} type="text" inputMode="numeric" autoComplete="one-time-code" aria-label={`Chiffre ${i + 1}`} value={digit} onChange={(e) => onDigitChange(i, e.target.value)} onKeyDown={(e) => onKeyDown(i, e)} className="w-11 h-12 text-center text-lg font-bold rounded-input border border-ink/15 bg-cream focus:outline-none focus:border-terracotta" />
                ))}
              </div>

              <button type="submit" disabled={busy || !otpComplet} className="w-full min-h-[52px] rounded-pill bg-terracotta hover:bg-terracotta-dark text-white font-bold text-sm inline-flex items-center justify-center gap-2 shadow-soft transition-colors disabled:opacity-60">
