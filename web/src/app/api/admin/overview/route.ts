...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const result of [salonsResult, rolesResult, activeSubscriptionsResult, paidPaymentsResult, recentSalonsResult, jobsResult, headsResult]) if (result.error) throw new Error(result.error.message);

    const planDistribution: Record<'PRO' | 'VIP' | 'EXTRA', number> = { PRO: 0, VIP: 0, EXTRA: 0 };
    for (const salon of salonsResult.data || []) if (salon.plan === 'PRO' || salon.plan === 'VIP' || salon.plan === 'EXTRA') planDistribution[salon.plan] += 1;
    const roleDistribution: Record<'customer' | 'salon' | 'admin', number> = { customer: 0, salon: 0, admin: 0 };
    for (const profile of rolesResult.data || []) if (profile.role === 'customer' || profile.role === 'salon' || profile.role === 'admin') roleDistribution[profile.role] += 1;
    ...
    for (const job of jobsResult.data || []) if (job.status in jobs) jobs[job.status as keyof typeof jobs] += 1;

    return NextResponse.json({
      kpis: { salons: salonsResult.data?.length ?? 0, users: rolesResult.data?.length ?? 0, activeSubscriptions: activeSubscriptionsResult.count ?? 0, paidTransactions: payments.length, totalRevenueFcfa, subscriptionRevenueFcfa, creditRevenueFcfa, canonicalHeads: headsResult.count ?? 0 },
      planDistribution,
      roleDistribution,
      jobs,
      paymentProviders: providerStates.map((state) => ({ ...state, ...(providerMetrics.get(state.provider) || { paidTransactions: 0, revenueFcfa: 0 }) })),
      recentSalons: recentSalons.map((salon) => ({ ...salon, status: activeSalonIds.has(salon.id) ? 'Actif' : 'Sans abonnement actif' })),
    });
  } catch (error) {
