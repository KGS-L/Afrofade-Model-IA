...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const portfolio = await Promise.all((portfolioResult.data ?? []).map(async (item: any) => { const signed = await supabase.storage.from(item.bucket).createSignedUrl(item.storage_path, 900); return { ...item, storage_path: undefined, bucket: undefined, imageUrl: signed.data?.signedUrl ?? null }; }));
    const affiliations = (membershipsResult.data ?? []).flatMap((membership: any) => { const salon = Array.isArray(membership.salons) ? membership.salons[0] : membership.salons; if (!salon || salon.verification_status !== 'verified' || salon.listing_status !== 'published') return []; return [{ id: salon.id, slug: salon.slug, name: salon.name, city: salon.city, neighborhood: salon.neighborhood, role: membership.role }]; });
