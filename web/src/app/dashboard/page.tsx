...
export default function DashboardPage() {
  ...
  const load = async (silent = false) => {
    ...
    setError(null);
    try {
      applyDashboard(await fetchSalonDashboard());
    } catch (loadError) {
  ...
  useEffect(() => {
    if (!hydrated || user?.role !== 'salon') return;
    ...
    const poll = async () => {
      ...
      try {
        ...
        // transient provider/webhook delay
      }
      if (attempts >= 15) {
        stopped = true;
        setPaymentSyncing(false);
    ...
    <div className="min-h-screen bg-cream text-ink">
      <header className="sticky top-0 z-30 bg-cream/95 backdrop-blur-md border-b border-ink/10">
        <div className="max-w-container mx-auto px-6 py-3 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-2"><Link href="/rituel" className="min-h-[44px] inline-flex items-center gap-2 bg-terracotta text-white font-bold text-sm px-4 rounded-pill"><Sparkles className="w-4 h-4" /> Rituel</Link><button onClick={logout} aria-label="Se déconnecter" className="w-11 h-11 rounded-pill bg-card border border-ink/15 flex items-center justify-center"><LogOut className="w-4 h-4" /></button></div>
        </div>
      </header>

      <main className="max-w-container mx-auto px-6 py-10 space-y-8">
        <div><p className="font-hand text-2xl text-terracotta">bon retour</p><h1 className="font-display text-3xl">{data?.salon.name || user.name}</h1><p className="text-sm text-ink-soft mt-1">Profil, quota, têtes et facturation viennent de Supabase.</p></div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          ].map((stat) => <div key={stat.label} className="bg-card rounded-card border border-ink/10 p-5 shadow-soft"><div className="text-[10px] uppercase tracking-[0.12em] font-bold text-ink-soft flex items-center gap-2"><stat.icon className="w-3.5 h-3.5 text-terracotta" /> {stat.label}</div><p className="font-display text-2xl mt-2">{stat.value}</p></div>)}
        </div>

        <section className="bg-card rounded-card border border-ink/10 p-6 sm:p-8 shadow-soft space-y-5">
          <div className="flex items-center justify-between gap-3 flex-wrap"><div><h2 className="font-display text-xl">Profil du salon</h2><p className="text-xs text-ink-soft mt-1">Ces informations sont persistées dans <code>public.salons</code>.</p></div><span className={`text-xs font-bold px-3 py-1.5 rounded-pill ${(data?.salon.profileCompletion ?? 0) === 100 ? 'bg-terracotta text-white' : 'bg-terracotta-wash text-terracotta-dark'}`}>{data?.salon.profileCompletion ?? 0} % complété</span></div>
        ...
        {data?.subscription ? (
          <section className="bg-card rounded-card border border-terracotta/40 p-6 shadow-soft"><h2 className="font-display text-xl">Abonnement actif</h2><p className="text-sm text-ink-soft mt-2">Plan <strong className="text-ink">{data.salon.plan}</strong> · {data.subscription.provider} · expire le <strong className="text-ink">{new Date(data.subscription.expires_at).toLocaleDateString('fr-FR')}</strong></p></section>
        ) : (
          <section className="bg-card rounded-card border border-ink/10 p-6 sm:p-8 shadow-soft space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{TERMS.map((item) => <button key={item.id} type="button" onClick={() => setTerm(item.id)} className={`rounded-card border p-4 text-left ${term === item.id ? 'border-terracotta bg-terracotta-wash' : 'border-ink/10'}`}><span className="block font-bold text-sm">{item.label}</span><span className="block text-xs text-ink-soft mt-1">{eligible ? item.hint : item.id === 'mensuel' ? item.hint : 'Profil complet + premier abonnement requis'}</span></button>)}</div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{PLANS.map((plan) => { const price = monthlyPrice(plan.amount, eligible ? activeTerm.discount : 0); return <div key={plan.name} className={`rounded-card border p-5 flex flex-col ${plan.popular ? 'border-terracotta bg-terracotta-wash/40' : 'border-ink/10'}`}><p className="text-xs font-bold uppercase tracking-[0.12em] text-ink-soft">{plan.name}</p><p className="font-display text-2xl mt-2">{formatFcfa(price)} <span className="text-xs font-sans text-ink-soft">FCFA/mois</span></p><p className="text-xs text-ink-soft mt-2">{activeTerm.label} · total calculé côté serveur</p><ul className="text-xs text-ink-soft mt-4 space-y-1.5 mb-4">{plan.features.slice(0, 3).map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><button onClick={() => void subscribe(plan.name)} disabled={Boolean(paying)} className="mt-auto min-h-[46px] rounded-pill bg-terracotta text-white font-bold disabled:opacity-50">{paying === plan.name ? 'Ouverture du paiement…' : `Choisir ${plan.name}`}</button></div>; })}</div>
          </section>
        )}

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <section className="bg-card rounded-card border border-ink/10 p-6 shadow-soft"><h2 className="font-display text-lg flex items-center gap-2"><Images className="w-4 h-4 text-terracotta" /> Dernières têtes</h2><div className="divide-y divide-ink/10 mt-4">{(data?.recentHeads || []).length === 0 ? <p className="py-4 text-sm text-ink-soft">Aucune tête enregistrée.</p> : data?.recentHeads.map((head) => <div key={head.id} className="py-3 flex items-center justify-between gap-3"><div><p className="text-sm font-bold">{head.client_name}</p><p className="text-xs text-ink-soft">{new Date(head.created_at).toLocaleDateString('fr-FR')}</p></div>{head.mesh_3d_url ? <a href={head.mesh_3d_url} className="text-xs font-bold text-terracotta hover:underline">Ouvrir le GLB</a> : <span className="text-xs text-ink-soft">En traitement</span>}</div>)}</div></section>
          <section className="bg-card rounded-card border border-ink/10 p-6 shadow-soft"><h2 className="font-display text-lg flex items-center gap-2"><CreditCard className="w-4 h-4 text-terracotta" /> Historique des paiements</h2><div className="divide-y divide-ink/10 mt-4">{(data?.payments || []).length === 0 ? <p className="py-4 text-sm text-ink-soft">Aucun paiement.</p> : data?.payments.map((payment) => <div key={payment.id} className="py-3 flex items-center justify-between gap-3"><div><p className="text-sm font-bold">{payment.product_id}</p><p className="text-xs text-ink-soft">{payment.provider} · {payment.status}</p></div><span className="text-sm font-bold">{formatFcfa(payment.amount_fcfa)} FCFA</span></div>)}</div></section>
        </div>
      </main>
    </div>
  );
