...
import { getDialCodeForCountry, getNationalityForCountry } from '@/lib/countries';
    ...
    <main className="min-h-screen bg-cream text-ink px-6 py-10">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center space-y-3">
          <div className="mx-auto w-12 h-12 rounded-card bg-terracotta text-white flex items-center justify-center">
          ...
          <p className="text-sm text-ink-soft max-w-2xl mx-auto">
            Ce choix adapte le tableau de bord, la facturation et les outils 3D. Vous pourrez toujours contacter l’équipe si votre activité évolue.
          </p>
        </div>
        ...
        {!profileType && (
          <div className="grid md:grid-cols-2 gap-5">
            <button onClick={() => setProfileType('customer')} className="text-left bg-card rounded-card border border-ink/10 p-6 shadow-soft hover:border-terracotta transition-colors">
              <div className="w-11 h-11 rounded-full bg-terracotta-wash text-terracotta flex items-center justify-center">
              ...
              <ul className="mt-5 space-y-2 text-sm">
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Crédits rechargeables, sans abonnement mensuel</li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Création et conservation de vos rendus 3D</li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Téléchargement et partage de votre look</li>
              </ul>
            ...
            <button onClick={() => setProfileType('salon')} className="text-left bg-night text-white rounded-card border border-white/10 p-6 shadow-soft hover:border-terracotta transition-colors">
              <div className="w-11 h-11 rounded-full bg-terracotta text-white flex items-center justify-center">
              ...
              <ul className="mt-5 space-y-2 text-sm">
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Abonnements PRO, VIP et EXTRA</li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Quota de reconstructions pour vos clients</li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-terracotta"/>Historique, facturation et gestion du salon</li>
              </ul>
        ...
        {profileType && (
          <form onSubmit={submit} className="max-w-2xl mx-auto bg-card rounded-card border border-ink/10 p-6 sm:p-8 shadow-soft space-y-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[.12em] text-terracotta">
                  {profileType === 'salon' ? 'Salon de coiffure' : 'Particulier'}
                </p>
                <h2 className="font-display text-2xl mt-1">Finaliser votre profil</h2>
              </div>
              <button type="button" onClick={() => setProfileType(null)} className="text-xs font-bold text-ink-soft hover:text-terracotta">Changer</button>
            </div>

            {profileType === 'customer' ? (
              <label className="block text-xs font-bold">Votre nom
                <input value={form.displayName} onChange={(e) => setForm({ ...form, displayName: e.target.value })} required className="mt-1.5 w-full min-h-[48px] rounded-input border border-ink/15 bg-cream px-4 font-normal" />
              </label>
            ) : (
              <label className="block text-xs font-bold">Nom du salon
                <input value={form.salonName} onChange={(e) => setForm({ ...form, salonName: e.target.value })} required className="mt-1.5 w-full min-h-[48px] rounded-input border border-ink/15 bg-cream px-4 font-normal" />
              </label>
            )}

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="block text-xs font-bold">Pays de résidence
                <CountrySelect value={form.country} onChange={handleCountryChange} required className="mt-1.5 w-full font-normal" />
              </label>
              <label className="block text-xs font-bold">Nationalité
                <NationalitySelect value={form.nationality} onChange={(nationality) => setForm({ ...form, nationality })} required className="mt-1.5 w-full font-normal" />
              </label>
            </div>

            <label className="block text-xs font-bold">Téléphone (Indicatif + Numéro)
              <PhoneInput
            ...
            <button disabled={saving} className="w-full min-h-[52px] rounded-pill bg-terracotta text-white font-bold disabled:opacity-50 inline-flex items-center justify-center gap-2">
              {saving ? <><Sparkles className="w-4 h-4 animate-pulse"/>Création de votre espace…</> : <><Users className="w-4 h-4"/>Accéder à mon espace</>}
            </button>
          </form>
        )}
      </div>
