...
import {
  ...
  Flag, Home, Images, ListChecks, Menu, Scissors, Settings, Star, UserRound, Users, WalletCards, X,
} from 'lucide-react';
...
export default function WorkspaceShell() {
  ...
  useEffect(() => {
    ...
    fetch('/api/workspace/contexts', { cache: 'no-store' }).then(async (response) => {
      if (!response.ok) throw new Error('contexts_failed');
  ...
  const active = useMemo(() => {
    ...
    if (requestedKey) {
      const match = contexts.find((context) => context.key === requestedKey);
      if (match) return match;
    }
    if (typeof window !== 'undefined') {
      const stored = window.localStorage.getItem(STORE_KEY);
      const match = contexts.find((context) => context.key === stored);
      if (match) return match;
    }
    return contexts.find((context) => context.key === 'personal') || contexts[0];
  ...
  return <div className="min-h-screen bg-cream text-ink sm:flex">
    <aside className="hidden sm:flex w-64 lg:w-72 shrink-0 min-h-screen border-r border-ink/10 bg-card p-4 lg:p-5 flex-col sticky top-0 h-screen">
      ...
      <div className="relative mt-6"><select aria-label="Changer d’espace" value={active.key} onChange={(event) => switchContext(event.target.value)} className="w-full appearance-none rounded-input border border-ink/10 bg-cream py-3 pl-3 pr-9 text-sm font-bold">{contexts.map((context) => <option key={context.key} value={context.key}>{context.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 w-4 h-4 text-ink-soft"/></div>
    ...
    {drawer && <div className="sm:hidden fixed inset-0 z-50 bg-ink/35" onClick={() => setDrawer(false)}><div className="h-full w-[88%] max-w-sm bg-card p-4 overflow-y-auto" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div className="font-display text-2xl">Afrofade</div><button onClick={() => setDrawer(false)} className="w-11 h-11 rounded-full flex items-center justify-center"><X className="w-5 h-5"/></button></div><div className="mt-5"><p className="text-xs font-bold uppercase tracking-[.12em] text-ink-soft mb-2">Changer d’espace</p><div className="space-y-2">{contexts.map((context) => <button key={context.key} onClick={() => switchContext(context.key)} className={`w-full text-left rounded-input border p-3 ${context.key === active.key ? 'border-terracotta bg-terracotta-wash' : 'border-ink/10'}`}><span className="font-bold text-sm">{context.label}</span>{context.type === 'salon' && <span className="block text-xs text-ink-soft mt-1">{context.role}</span>}</button>)}</div></div><div className="mt-6 border-t border-ink/10 pt-4 space-y-1">{nav.map((item) => <button key={item.id} onClick={() => go(item)} className="w-full min-h-[46px] rounded-input px-3 flex items-center gap-3 text-sm text-left hover:bg-cream"><item.icon className="w-4 h-4"/>{item.label}</button>)}</div></div></div>}
  </div>;
}
