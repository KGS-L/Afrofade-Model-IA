...
- `demo_video_autoplay` (landing `#rituel-studio`) : mockup vidéo auto-play en boucle, muet — viseur du scan montrant les photos du personnage client (flux caméra simulé, léger travelling) avec ovale, consignes, flash de capture et filmstrip qui se remplit → overlay reconstruction → rendus 3D d'essayage du hero qui défilent. Pas d'avatar procédural (jugé peu réaliste, décision Jonas-dev 2026-08-18). Aucune interaction ; poster statique sous `prefers-reduced-motion`.
