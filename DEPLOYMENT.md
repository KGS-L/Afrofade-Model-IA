...
Ce document décrit la procédure automatisée de déploiement en production pour **Afrofade 3D Studio** via le pipeline CI/CD GitHub Actions (`environment: prod`) et les scripts d'installation VPS dans le dossier `projects/Afrofade`.
...
| **`VPS_AFROFADE_PATH`** | Chemin absolu du projet sur le VPS                     | `/home/admin/projects/Afrofade` |
...
```yaml
deploy-vps:
  name: Deploy Afrofade to VPS (projects/Afrofade)
  environment: prod
...
1. **Copie les fichiers** et manifestes dans `/home/admin/projects/Afrofade`.
...
```bash
cd ~/projects/Afrofade
export VPS_SUDO_PASSWORD="votre_mot_de_passe"
./scripts/deploy-vps.sh ~/projects/Afrofade afrofade.pro contact@afrofade.pro
```
