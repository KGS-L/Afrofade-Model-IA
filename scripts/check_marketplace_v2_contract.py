...
must('web/supabase/migrations/40_marketplace_booking_foundation.sql','marketplace_bookings_no_professional_overlap')
...
if '/admin/marketplace' not in workspace: fail('admin workspace must link to marketplace moderation')
must('web/src/components/marketplace/BookingWizard.tsx','Continuer pour confirmer')
must('web/src/components/marketplace/BookingWizard.tsx',"event:'booking_created'")
must('web/src/components/marketplace/PublicProviderProfile.tsx','Avis vérifiés')
must('web/src/components/marketplace/PublicProviderProfile.tsx',"event:'provider_view'")
must('web/src/components/marketplace/DiscoverMarketplace.tsx',"event:'search'")
must('web/src/components/careers/CareersManager.tsx','Inviter dans l’équipe')
must('web/src/components/careers/CareersMarketplace.tsx',"event:'job_application'")
must('web/src/components/workspace/BusinessOverview.tsx','Business actif')
must('web/src/components/workspace/SubscriptionWorkspace.tsx',"event:'subscription_checkout_started'")
must('web/src/lib/marketplace-plans.ts',"positiveEnvInt('PROFESSIONAL_PRO_PRICE_FCFA')")
must('web/src/lib/marketplace-telemetry.ts','trackMarketplaceEvent')
must('web/src/app/api/admin/marketplace/moderation/route.ts','admin_moderation_actions')
must('web/src/components/admin/MarketplaceModerationConsole.tsx','Modération Marketplace')
tracker=must('_bmad-output/implementation-artifacts/marketplace-sprint-status.yaml','cross-epic-review-12-to-17')
if 'review_pending_ci' not in tracker: fail('release gates must remain pending CI during review')
if 'review_feature_gated' not in tracker: fail('future payment/sponsored stories must remain feature gated')
